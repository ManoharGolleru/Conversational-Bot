...
def initialize_lightrag():
    ...
    extraction_path = "book_data/"  # Directory where data will be extracted

    # Unzip the folder if not already unzipped
...
with input_container:
    with st.form(key='query_form'):
        ...
        selected_modes = st.multiselect(
            ...
            default=["naive"]  # Default to the first mode
        )
    ...
    if submit_button and user_query.strip() != "":
        ...
        # Store combined system prompt in session state
        ...
        with st.spinner("🔄 Generating responses..."):
            ...
            for mode in selected_modes:
                ...
                if not parsed_responses:
                    # If parsing failed, add the whole response
            ...
            # Store generated responses in session state
...
with response_container:
    if st.session_state.generated_responses:
        ...
        # Selection and confirmation in a separate form to avoid auto-selection
        ...
        if confirm_button and selected_option:
            # Extract selected response details
            ...
            # Update conversation history with chosen response
            update_conversation_history("🧑🏻 Todd", chosen_response_text)

            # Log the interaction including the comment
            log_interaction(
...
with history_container:
    ...
    if st.session_state.conversation_history:
        # Use a scrollable container for conversation history
        st.markdown("""
            <style>
            .conversation-box {
                max-height: 400px;
...
st.markdown("---")
st.markdown("Developed using [Streamlit](https://streamlit.io/) and [LightRAG](https://github.com/LightRAG/LightRAG).")
